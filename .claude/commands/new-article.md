---
description: KWから記事を全自動制作しShopify下書きまで作る（公開のみ手動）
argument-hint: [キーワード（省略可：省略時はKW選定から）]
---

あなたはSOLSTARのSEO記事制作パイプラインの中央指揮役です。以下を順に実行し、Shopifyの下書き作成まで自動で進めてください。**公開は絶対にしない**（人間が管理画面で行う）。`CLAUDE.md` のルールを全工程で遵守すること。

対象キーワード: $ARGUMENTS

## 実行手順

### 0. KW決定
- 引数でキーワードが指定されていれば、それを使う。
- 指定がなければ `keyword-strategist` サブエージェントを起動し、Ahrefsリスト×GSC×3Cで最有力KWを1つ自動選定する（候補と選定理由を簡潔に提示してから進む）。

### 1. 設計（③）
- `article-designer` サブエージェントを起動し、設計ブリーフ `drafts/<handle>-brief.md` を作らせる。
- ※今回は「公開のみ手動」方針のため、構成承認の停止はしない。設計内容は要約だけ提示して次へ進む。
- 価格心理・EC・ブランディング系テーマなら、kolendaガイドを参考資料に（翻訳転載はせず原理を抽出・出典明記）。

### 2. 執筆（④）
- `article-writer` サブエージェントを起動。`article-template.html`・`company-facts.md`・ブリーフに沿って本文HTMLを `drafts/<handle>.html` に書かせる。
- SOLSTAR固有の事実は `company-facts.md` の範囲のみ。無い事実は創作せず `【要記入：…】` を残す。

### 3. 校閲（④.5・日本語ネイティブ編集）
- `japanese-editor` サブエージェントを起動し、`drafts/<handle>.html` の **本文テキストのみ** を日本人が自然に読める文章へ編集させる。
- 構成・HTML・目次・JSON-LD・SEOキーワード・出典は変更しない（AI感・翻訳調の排除が目的）。

### 3.5. 法務ゲート（④.7・95点／校閲の直後）
- `legal-reviewer` サブエージェントを起動し、`drafts/<handle>.html` を日本の広告・表示関連法（景品表示法・ステマ規制・薬機法・特定商取引法・著作権法など）の観点で採点させる。
- **95点未満、または高リスク表現が1つでも残る場合は、指摘を `article-writer` に差し戻してRewrite**（表現変更で本文が変わるため、必要に応じ `japanese-editor` で再校閲）→ `legal-reviewer` で再審査。**95点以上になるまで繰り返す**（最大3周）。
- 3周しても未達なら、残った法的リスクを添えて人間に報告して停止（公開には進まない）。
- 法務ゲートを通過してから、次の品質ゲートへ進む。

### 4. 品質ゲート（⑤・95点）
- `article-reviewer` を **3観点で並行起動**（focus: SEO観点 / 読者・E-E-A-T観点 / AI感・独自性観点）。
- 3点の平均が **95点未満なら、指摘を統合して `article-writer` に差し戻し改稿**（必要に応じ `japanese-editor` で再校閲）→ 再審査。合格まで繰り返す（最大3周）。
- 3周しても未達なら、残課題を添えて人間に報告して停止（公開には進まない）。

### 4.5. 合格後の追加校閲（日本語精度の磨き込み・3回）
- 95点に達した後も、**日本語の精度をさらに上げるため `japanese-editor` を追加で3回繰り返し起動**する（モード3「合格後の追加校閲」として指示）。
- 各回とも本文テキストのみを磨き込む（語尾のリズム・翻訳調・冗長表現・漢字/用語の難易度）。構成・HTML・SEOキーワード・事実・出典は変更しない。**法的リスクのある表現（断定・保証・最上級・誇大・無根拠No.1など）を新たに作らない**。
- 3回完了後、`article-reviewer`（品質）と `legal-reviewer`（法務）の**両方を再走**させ、**ともに95点以上を維持**していることを確認する。品質が落ちたら手順4（品質ゲート）へ、法務が落ちたら手順3.5（法務ゲート）へ差し戻す。

### 5. 公開＝下書き作成（⑥）
- 合格を維持したら `article-publisher` サブエージェントを起動し、Shopifyに **isPublished:false の下書き** として作成。
- JSON-LDのプレースホルダ（HEADLINE/DESCRIPTION/PAGE_URL/DATE_PUBLISHED/DATE_MODIFIED/BLOG_NAME/BLOG_URL）を確定値に置換してから投稿（置換の最終責任は publisher。writer が先に埋めた HEADLINE/DESCRIPTION もブリーフの確定値で上書き確認する）。

### 6. 報告
- 最終の品質スコア、作成した下書きのadmin URL、`【要記入】`が残っていればその一覧、公開前に人間が確認すべき点（プレビュー・アイキャッチ画像・内部リンク）を報告する。
- **公開はしていないこと**を明記し、人間に最終公開を委ねる。

## 重要な原則
- 公開は人間のみ。自動公開は禁止。
- 事実の創作禁止（company-facts.md の範囲＋一般論のみ）。
- kolenda等の他社コンテンツは翻訳転載しない（参考にしてオリジナル執筆＋出典明記）。
- このコマンドは `~/shopify-articles/` から実行する前提（サブエージェントが名前で解決される）。
