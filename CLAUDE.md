...
| Branding | branding | gid://shopify/Blog/103031931102 | ブランディング・ブランド論系コラム |
| News | news | gid://shopify/Blog/93445423326 | お知らせ（現在未使用） |

テーマに応じて適切なブログを選ぶ。SEOコラムは原則 Shopify か Marketing。ブランディング・ブランド論に寄るテーマは Branding を検討する。
...
- 各セクションは結論→理由→具体例の順。出典や調査結果をもとに書く場合も、原文の情報順や分量に引っ張られず、まず読者が知りたい結論・要点を1〜2文で示してから、詳細な根拠・出典情報を続ける。
- 情報量が多い部分（項目・手順・条件・数値などが3つ以上並ぶ場合）は、地の文で羅列せず箇条書きや表に変換して読みやすくする。
...
- FAQは「まとめ」の直前に3〜5問程度で設置する。各質問のH3見出しは必ず先頭に「Q.」を付ける（例: `<h3>Q. 〇〇はどのくらいですか？</h3>`）。回答本文に「A.」は付けない。
